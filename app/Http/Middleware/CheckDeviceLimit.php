...
class CheckDeviceLimit
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $device = UserDevice::where('user_id', $user->id)->where('device_id', $sessionDeviceId)->first();
        ...
        if (!$device) {
            ...
            if (!$device) {
                Auth::logout();
                return redirect()->route('login')->withErrors('Anda telah mencapai batas penggunaan perangkat');
            }
        }
